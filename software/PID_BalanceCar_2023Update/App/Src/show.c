...
void oled_show(void)
{
	...
	if (BalanceCar.Flag_follow == 1)
		OLED_ShowString(70, 0, "Follow ");
	else if (BalanceCar.Flag_avoid == 1)
		OLED_ShowString(70, 0, "Avoid  ");
	else
		OLED_ShowString(70, 0, "Balance");
	//=============第二行显示角度=======================//
	OLED_ShowString(00, 10, "Angle");
	if (BalanceCar.Angle_Balance < 0)
		OLED_ShowString(48, 10, "-");
	if (BalanceCar.Angle_Balance >= 0)
		OLED_ShowString(48, 10, "+");
	OLED_ShowNumber(56, 10, myabs((int)BalanceCar.Angle_Balance), 3, 12);
	//=============第三行显示角速度与距离===============//
	OLED_ShowString(0, 20, "Gyrox");
	if (BalanceCar.Gyro_Balance < 0)
		OLED_ShowString(42, 20, "-");
	if (BalanceCar.Gyro_Balance >= 0)
		OLED_ShowString(42, 20, "+");
	OLED_ShowNumber(50, 20, myabs((int)BalanceCar.Gyro_Balance), 4, 12);

	OLED_ShowNumber(82, 20, (u16)BalanceCar.Distance, 5, 12);
	OLED_ShowString(114, 20, "mm");

	//=============第四行显示左编码器PWM与读数=======================//
	OLED_ShowString(00, 30, "L");
	if (BalanceCar.Motor_Left < 0)
		OLED_ShowString(16, 30, "-"),
			OLED_ShowNumber(26, 30, myabs((int)BalanceCar.Motor_Left), 4, 12);
	if (BalanceCar.Motor_Left >= 0)
		OLED_ShowString(16, 30, "+"),
			OLED_ShowNumber(26, 30, myabs((int)BalanceCar.Motor_Left), 4, 12);
	...
	if (BalanceCar.Motor_Right < 0)
		OLED_ShowString(16, 40, "-"),
			OLED_ShowNumber(26, 40, myabs((int)BalanceCar.Motor_Right), 4, 12);
	if (BalanceCar.Motor_Right >= 0)
		OLED_ShowString(16, 40, "+"),
			OLED_ShowNumber(26, 40, myabs((int)BalanceCar.Motor_Right), 4, 12);
	...
	OLED_ShowNumber(19, 50, BalanceCar.Voltage / 100, 2, 12);
	OLED_ShowNumber(42, 50, BalanceCar.Voltage % 100, 2, 12);
	if (BalanceCar.Flag_Stop)
